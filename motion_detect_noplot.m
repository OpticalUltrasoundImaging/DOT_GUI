function [tar_isMotion, ref_isMotion] = motion_detect_noplot(tar_data_path, ref_data_path, tar_nLesion, ref_nLesion, meanTol, ptDiffTol, badptsTol, source, detect, sddist_matrix, tar_alldata, ref_alldata)
...
%     tar_std_23 = zeros(4,1);
    for wavidx = 1:4
        ...
        tar_var_23(wavidx) = std2(ad23)*std2(ad23);
...
%         tar_std_23(wavidx) = std2(ad23);
    ...
    if idx == tar_nLesion
        for i = 1:length(tar_isMotion)/2
            if tar_isMotion(2*i-1) == 1 || tar_isMotion(2*i) == 1
                motion_sum = motion_sum+ 1;
            end
        end
        if motion_sum > 0.8 *(length(tar_isMotion)/2)
...
%     ref_std_23 = zeros(4,1);
    for wavidx = 1:4
        ...
        ref_var_23(wavidx) = std2(ad23)*std2(ad23);
        
%         ref_std_12(wavidx) = std2(ad12);
%         ref_std_23(wavidx) = std2(ad23);
        
        ref_Features(idx*2-1, wavidx) = ref_mean_12(wavidx);
        ref_Features(idx*2, wavidx) = ref_mean_23(wavidx);
        ref_Features(idx*2-1, 46+wavidx) = ref_var_12(wavidx);
        ref_Features(idx*2, 46+wavidx) = ref_var_23(wavidx);
    end
    ...
    if idx == ref_nLesion
        for i = 1:length(ref_isMotion)/2
            if ref_isMotion(2*i-1) == 1 || ref_isMotion(2*i) == 1
                motion_sum = motion_sum+ 1;
            end
        end
        if motion_sum > 0.8 *(length(ref_isMotion)/2)
